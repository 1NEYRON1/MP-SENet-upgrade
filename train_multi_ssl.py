"""
Training script with Multi-Scale Discriminator + SSL Loss.
Combines:
- Multi-Scale Discriminator operating on multiple STFT resolutions
- SSL (Self-Supervised Learning) loss using NVIDIA NeMo for embedding similarity

CUDA_VISIBLE_DEVICES=1 python train_multi_ssl.py    --config config.json    --checkpoint_path cp_model_multi_ssl_16_seg    --ssl_model_name nvidia/ssl_en_nest_xlarge_v1.0    --ssl_loss_weight 0.1
...
def train(rank, a, h):
    ...
    # Multi-Scale Discriminator with configurable resolutions
    resolutions = getattr(h, 'discriminator_resolutions', [[400, 100, 400], [1024, 120, 600], [256, 50, 120]])
    discriminator = MultiScaleDiscriminator(resolutions=resolutions, compress_factor=h.compress_factor).to(device)
    
    # Initialize SSL Loss module
    ssl_model_name = getattr(h, 'ssl_model_name', 'nvidia/ssl_en_nest_xlarge_v1.0')
    ssl_loss_weight = getattr(h, 'ssl_loss_weight', 0.1)
    
    if rank == 0:
        print(f"Initializing SSL Loss with model: {ssl_model_name}")
        print(f"SSL Loss weight: {ssl_loss_weight}")
    
    ssl_loss_module = SSLLossSimple(model_name=ssl_model_name, device=device)

    if rank == 0:
        print(generator)
        num_params_g = sum(p.numel() for p in generator.parameters())
        num_params_d = sum(p.numel() for p in discriminator.parameters())
        print('Generator Parameters: {:.3f}M'.format(num_params_g/1e6))
        print('Discriminator Parameters: {:.3f}M (Multi-Scale with {} resolutions)'.format(
            num_params_d/1e6, len(resolutions)))
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            for scale_idx in range(num_resolutions):
                # Real samples should output 1
                loss_disc_r += F.mse_loss(one_labels, metric_rs[scale_idx].flatten())
                
                # Generated samples should output PESQ score (normalized)
                if batch_pesq_score is not None:
                    loss_disc_g += F.mse_loss(batch_pesq_score.to(device), metric_gs[scale_idx].flatten())
            
            # Average over scales
            loss_disc_r = loss_disc_r / num_resolutions
            if batch_pesq_score is not None:
                loss_disc_g = loss_disc_g / num_resolutions
            else:
            ...
            # Multi-Scale Metric Loss
            _, metric_gs = discriminator(clean_audio, audio_g)
            loss_metric = 0
            for scale_idx in range(num_resolutions):
                loss_metric += F.mse_loss(metric_gs[scale_idx].flatten(), one_labels)
            loss_metric = loss_metric / num_resolutions
            
            # SSL Loss - embedding similarity between clean and enhanced audio
            loss_ssl = ssl_loss_module(clean_audio, audio_g)

            # Total Generator Loss with Multi-Scale Discriminator + SSL
            ...
            if rank == 0:
                ...
                if steps % a.stdout_interval == 0:
                    with torch.no_grad():
                        ...
                        ssl_error = loss_ssl.item()
                    print('Steps : {:d}, Gen Loss: {:4.3f}, Disc Loss: {:4.3f}, Metric loss: {:4.3f}, '
                          'Mag Loss: {:4.3f}, Pha Loss: {:4.3f}, Com Loss: {:4.3f}, Time Loss: {:4.3f}, '
                          'STFT Loss: {:4.3f}, SSL Loss: {:4.3f}, s/b: {:4.3f}'.
                           format(steps, loss_gen_all, loss_disc_all, metric_error, mag_error, pha_error, 
                                  com_error, time_error, stft_error, ssl_error, time.time() - start_b))
                ...
                if steps % a.summary_interval == 0:
                    ...
                    sw.add_scalar("Training/Metric Loss (Multi-Scale)", metric_error, steps)
                ...
                if steps % a.validation_interval == 0 and steps != 0:
                    ...
                    with torch.no_grad():
                        for j, batch in enumerate(validation_loader):
                            ...
                            val_ssl_err_tot += ssl_loss_module(clean_audio, audio_g).item()
                        ...
                        val_ssl_err = val_ssl_err_tot / (j+1)
                        val_pesq_score = pesq_score(audios_r, audios_g, h).item()
                        print('Steps : {:d}, PESQ Score: {:4.3f}, Val SSL Loss: {:4.3f}, s/b : {:4.3f}'.
                                format(steps, val_pesq_score, val_ssl_err, time.time() - start_b))
                    ...
                    if epoch >= a.best_checkpoint_start_epoch:
                        if val_pesq_score > best_pesq:
                            ...
                            save_checkpoint(best_checkpoint_path,
                                        {'generator': (generator.module if h.num_gpus > 1 else generator).state_dict()})
                            print(f"New best PESQ: {best_pesq:.4f}, saved to {best_checkpoint_path}")
...
def main():
    print('Initializing Training Process with Multi-Scale Discriminator + SSL Loss..')
    ...
    parser.add_argument('--checkpoint_path', default='cp_model_multi_ssl')
    ...
    parser.add_argument('--ssl_model_name', default='nvidia/ssl_en_nest_xlarge_v1.0', 
                        help='NVIDIA NeMo SSL model name')
    parser.add_argument('--ssl_loss_weight', default=0.1, type=float,
                        help='Weight for SSL loss in total generator loss')
