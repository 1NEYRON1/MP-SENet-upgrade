...
def wss(clean_speech, processed_speech, sample_rate):
    ...
    _USE_FFT_SPECTRUM = 1  # defaults to 10th order LP spectrum
...
def snr(clean_speech, processed_speech, sample_rate):
    ...
    overall_snr = 10 * np.log10(
        np.sum(np.square(clean_speech)) / np.sum(np.square(clean_speech - processed_speech))
    )
...
def thirdoct(fs, N_fft, numBands, mn):
    ...
    fl = np.sqrt(
        np.multiply(np.multiply(np.power(2, k / 3), mn), np.multiply(np.power(2, (k - 1) / 3), mn))
    )
    fr = np.sqrt(
        np.multiply(np.multiply(np.power(2, k / 3), mn), np.multiply(np.power(2, (k + 1) / 3), mn))
    )
...
def stdft(x, N, K, N_fft):
    ...
    x_stdft = signal.stft(
        x, window=w, nperseg=N, noverlap=K, nfft=N_fft, return_onesided=False, boundary=None
    )[2]
